...
import com.cwpark.petmap.petmap.data.response.ErrorResponse;
...
public class GlobalExceptionHandler {
  ...
  protected ResponseEntity<ErrorResponse> handleEmptyResultDataAccessException(EmptyResultDataAccessException ex) {
    ...
    final ErrorResponse response = ErrorResponse.of(ErrorCode.EMPTYRESULT_DATAACCESS_ERROR, ex.getMessage());
    return new ResponseEntity<>(response, HTTP_STATUS_OK);
  }

  @ExceptionHandler(DataIntegrityViolationException.class)
  protected ResponseEntity<ErrorResponse> dataIntegrityViolationException(DataIntegrityViolationException ex) {
    log.error("DataIntegrityViolationException", ex);
    final ErrorResponse response = ErrorResponse.of(ErrorCode.DATAINTEGRITY_VIOLATION_ERROR, ex.getMessage());
    return new ResponseEntity<>(response, HTTP_STATUS_OK);
  }

  @ExceptionHandler(CountException.class)
  protected ResponseEntity<ErrorResponse> CountException(CountException ex) {
    log.error("CountException", ex);
    final ErrorResponse response = ErrorResponse.of(ErrorCode.COUNT_ERROR, ex.getMessage());
    return new ResponseEntity<>(response, HTTP_STATUS_OK);
  }

  @ExceptionHandler(NoSuchDataException.class)
  protected ResponseEntity<ErrorResponse> NoSuchDataException(NoSuchDataException ex) {
    log.error("NoSuchDataException", ex);
    final ErrorResponse response = ErrorResponse.of(ErrorCode.NOSUCHDATA_ERROR, ex.getMessage());
    return new ResponseEntity<>(response, HTTP_STATUS_OK);
  }
